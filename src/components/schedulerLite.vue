          ...
          <div class="sc-main-scroll" :style="{ width: state.contentW + 'px' }">
            <div class="sc-main">
                ...
                >
                    ...
                    >
                    {{ (getHeaderMonth(n - 1) !== getHeaderMonth(n - 2)) ||(getHeaderMonth(n - 1) === getHeaderMonth(n - 2) && n === 1) ? getHeaderMonth(n - 1) : "" }}
                    </div>
                </div>
              <div
                class="timeline"
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const getHeaderDate = (n) => {
        ...
        return dateFormatter(startDate, true, false);
      };

      /**
       * Get header area month-text
       * 
       * @param Int n Col index
       *
       * @returns String
       */
      const getHeaderMonth = (n) => {
        let startDate = addDays(new Date(state.settingData.startDate), n);
        return dateFormatter(startDate, false, true);
      }
       ...
       * @returns String
       */
      const dateFormatter = (dateObj, withWeekDay, withMonth) => {
      ...
      const selectStartTime = (rowIndex, keyIndex) => {
        // console.log("editClick in selectStartTime");
        ...
        let msg = prompt("Please enter your message:", "New");
        if (msg == null || msg == "") {
            textMsg = "New";
        } else {
            textMsg = msg;
      ...
      const editScheduleData = (rowIndex, keyNo, unitCnt) => {
        console.log("editClick in editScheduleData");
        let targetData = props.scheduleData[rowIndex].schedule[keyNo];
        if (targetData) {
          let changeDatetimeText = (datetimeText) => {
            let addMinutes1 = unitCnt * state.settingData.unit;
      ...
      state.padding =
        state.settingData.dateDivH * 2 +
        1 +
        state.settingData.borderW * 2;
      state.dateDivW =
